...
> **Python 3.10+ 필요**. `bootstrap.sh` 는 현재 `python3` 로 먼저 시도하고, PyPI 패키지가 아직 최신 Python 을 못 따라온 경우 설치된 `python3.13`/`3.12`/`3.11`/`3.10` 후보로 자동 fallback 한다. Docker가 동작해야 통신 백본이 선다
...
- **언어 / 런타임** — Python 3.10+ (최신 Python 우선 시도, 패키지 호환 실패 시 설치된 안정 Python 후보로 venv 자동 재생성)
- **두뇌** — Nous Research `hermes-agent` (Python 3.10은 0.15.x, Python 3.11+는 PyPI 최신 호환 버전), 모델은 OpenRouter DeepSeek V4 Flash(기본) 또는 로컬 Ollama 등 OpenAI 호환 서버
