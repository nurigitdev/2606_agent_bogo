...
def react_system_addendum(max_steps):
    ...
    return (
        ...
        "- ★모르면 먼저 도구로 조사한다(되묻기 전에). 사장/상위가 자료·검토를 요청하면 "
        "self_critique 단계에서 '결론·옵션(소요·리스크)·추천·예상질문 선답·후속안'이 다 찼는지 "
        "스스로 점검하고, 빈 칸은 도구 조사 또는 업계 통상치 추정('(추정: 근거)' 표시)으로 채운 뒤 "
        "finalize 하라. '옵션/소요/결정항목을 채워 달라'고 사장에게 되묻고 끝내는 finalize 는 임무 "
        "포기다 — 그 채움이 곧 네 일이다(공통규칙 §2-1).\n"
        "- 송신할 게 없으면 finalize(act=false)로 즉시 끝내라(침묵도 유효한 결정). 단, 자료 요청에 "
        "대한 '되묻기'는 침묵이 아니라 미완성 송신이므로 위 자율 완결을 먼저 채워라.\n"
...
def finalize_schema_instruction():
    ...
    return (
        ...
        "- ★자율 완결(공통규칙 §2-1): 사장/상위가 자료·검토를 요청하면 message 안에서 "
        "결론·옵션(소요·리스크 포함)·추천·예상질문 선답·후속안까지 스스로 다 채워라. "
        "모르는 항목은 읽기 도구로 조사하고, 못 구하면 업계 통상치로 추정해 '(추정: 근거)'로 "
        "표시한다. '옵션/소요/결정항목을 채워 달라'고 사장에게 되묻고 끝내지 마라 — 그게 곧 네 일이다. "
        "되물음만 담은 message 는 미완성이다(비가역 사실 1~2개만 확인이 필요하면 그것만 콕 집되 "
        "나머지는 다 채워 같은 message 에 담아라).\n"
