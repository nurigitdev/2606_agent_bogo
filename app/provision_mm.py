...
def _looks_like_token(s):
    """MM Personal Access Token 모양(26자 이상 소문자+숫자 영숫자)인지."""
    return bool(s) and len(s) >= 26 and s.isalnum()


def _extract_token(text):
    """mmctl 출력(텍스트/JSON 혼재)에서 Access Token 1개를 뽑는다.

    WHY (근본): mmctl 의 실제 출력 형식은 토큰이 콜론 '앞'에 온다(신선 컨테이너 실측).
      - `bot create --with-token`(텍스트):  "Created bot <id>\\n<TOKEN>: autogenerated"
      - `token generate <user> <desc>`(텍스트): "<TOKEN>: <desc>"
      - 두 명령의 `--json`: 리스트 [ {...객체...}, {"token": "<TOKEN>", ...} ]
    이전 파서는 (a) JSON 을 dict 로만 처리해 리스트(mmctl 실제 반환)를 놓치고,
    (b) 텍스트를 split(":")[-1] 로 잘라 '콜론 뒤 설명'을 토큰으로 오인 → 항상 실패했다.
    이제 JSON(dict/list) 우선 + 텍스트는 콜론 양측 모든 조각을 스캔해 포맷에 불변이게 한다.
    """
    # 1) JSON 우선(가장 안정된 계약). dict 또는 list(mmctl 실제 반환) 모두 처리.
    try:
        j = json.loads(text)
    ...
    for obj in candidates:
        t = obj.get("token") or (obj.get("data") or {}).get("token")
        if t:
            return t
    # 2) 텍스트 폴백: 각 라인을 콜론으로 나눠 '모든 조각'에서 토큰 모양을 찾는다
    #    (토큰이 콜론 앞/뒤 어디에 오든, "Token: xxx"·"<token>: desc" 모두 대응).
    #    단, "Created bot <id>" 라인은 봇 id(26자 영숫자)가 토큰과 구분 안 돼 오인되므로
    #    건너뛴다(신선 컨테이너 실측 회귀). JSON 경로가 실패한 예외적 폴백일 때만 도달.
    for line in text.splitlines():
        low = line.strip().lower()
        if low.startswith("created bot") or low.startswith("bot "):
            continue
...
def ensure_bot(config_stem, username, display_name):
    ...
    # WHY --json: 텍스트 출력은 "Created bot <id>\n<TOKEN>: autogenerated" 형식이라
    #   첫 라인의 봇 id(26자 영숫자)가 토큰과 구분되지 않아 파서가 id 를 토큰으로 오인한다
    #   (신선 컨테이너 실측). --json 은 [{...봇...}, {"token": "<TOKEN>",...}] 구조로
    #   토큰이 명시 필드라 오인이 원천 불가능하다. _extract_token 이 list 를 처리한다.
    bots = mmctl_auth_json("bot", "list") or []
    created_token = None
    if not _exists(bots, "username", username):
        out = mmctl_auth_json("bot", "create", username,
                              "--display-name", display_name, "--with-token")
        created_token = _extract_token(json.dumps(out)) if out is not None else None
        if not created_token:
            raise RuntimeError(f"봇 '{username}' 생성 직후 토큰 추출 실패(bot create --json).")
    ...
    #    --json 경로: 텍스트는 "<TOKEN>: <desc>"(토큰이 콜론 앞)라 파서 오인 위험이 있어
    #    JSON 명시 필드를 쓴다([{"token": "<TOKEN>",...}]).
    if not new_tok:
        out = mmctl_auth_json("token", "generate", username, "bogo-runtime")
        new_tok = _extract_token(json.dumps(out)) if out is not None else None
    if not new_tok:
