@echo off
rem ========================================================================
rem  BOGO 시작 (Windows) — 탐색기 더블클릭 1회로 3개 역할 + CEO 관리봇 상시 가동
rem ------------------------------------------------------------------------
rem  WHAT  이 파일을 더블클릭하면:
rem    1) 미등록  -> bogo_ctl.ps1 setup   (venv+의존성+config+Task Scheduler 등록)
rem    2) 등록됨  -> bogo_ctl.ps1 restart (최신 코드 재배포 + 4역할 재시작)
rem    3) 끝나면 현재 상태(Task State) 표시
rem  mac 'BOGO 시작.command' 의 Windows 등가물. 상시 가동은 Task Scheduler 담당.
...
set "LAUNCHER=%~dp0BOGO 시작.launcher.ps1"

rem -- 런처 본체 존재 확인 --
if not exist "%LAUNCHER%" (
  echo [오류] 런처 파일을 찾지 못했습니다: "%LAUNCHER%"
  echo        BOGO 시작.bat 와 BOGO 시작.launcher.ps1 은 같은 폴더에 함께 있어야 합니다.
  goto :hold
)
...
echo.
echo [BOGO] Windows 런처를 시작합니다 ^(%PS%^)...
echo.
