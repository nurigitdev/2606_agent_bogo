# ════════════════════════════════════════════════════════════════════════
#  BOGO 시작 (Windows 런처 본체) — .bat 이 호출하는 감지·분기 로직
# ════════════════════════════════════════════════════════════════════════
#  WHAT  더블클릭 → 이 스크립트가:
#    1) 미등록 → bogo_ctl.ps1 setup (venv+의존성+config+Task Scheduler 등록+기동)
#    2) 이미 상시 가동 중 → bogo_ctl.ps1 restart (최신 코드 재배포 + 4역할 재시작)
#    3) 끝나면 현재 상태(Task State)를 표시
#  설계 근거: mac 'BOGO 시작.command' 와 1:1 등가. 상시 가동은 Task Scheduler 가
#    담당하므로, 매번 pwsh ./bogo_ctl.ps1 {setup|restart|status} 를 타이핑하던
...
Write-Host ""
Say "BOGO 상시 가동 런처 시작 (Windows)"
Say "위치: $Repo"
Write-Host ""

# ── 1. 사전 점검: 컨트롤러 + PowerShell 호스트 확인 ─────────────────────
if (-not (Test-Path $Ctl)) {
  Fail "bogo_ctl.ps1 을 찾지 못했습니다: $Ctl"
...
# bogo_ctl.ps1 한 명령을 자식 PowerShell 로 호출 (ExecutionPolicy 우회 + 인코딩 보존)
...
#   Task Scheduler 에 BOGO_* 작업이 하나라도 있으면 = 이미 설치/등록됨.
#   (mac launchctl list | grep com.bogo 의 Windows 등가)
...
if ($RegisteredCount -ge 1) {
  ...
  if ($rc -eq 0) {
    ...
    Say "수동 진단:  cd `"$Repo`"; .\bogo_ctl.ps1 status"
    Stop-With 1
  }
} else {
  ...
  if ($rc -eq 0) {
    ...
    Say "흔한 원인: Python 3 미설치 → https://www.python.org/downloads/ 에서 설치"
...
if ($After.Count -ge 1) {
  ...
  Fail "등록된 BOGO 역할이 없습니다. 위 로그에서 원인을 확인하세요."
  Stop-With 1
}
